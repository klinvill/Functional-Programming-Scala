...
//      if (isStanding) {
//        (dy(-2, -1), Left)::(dy(1, 2), Right)::(dx(-2, -1), Up)::(dx(1, 2), Down)::List()
//      }
//      else if (b1.x == b2.x) {
//        (dy(-2, -1), Left)::(dy(1, 2), Right)::(dx(-1, -1), Up)::(dx(1, 1), Down)::List()
//      }
//      else {
//        (dy(-1, -1), Left)::(dy(1, 1), Right)::(dx(-2, -1), Up)::(dx(1, 2), Down)::List()
//      }
//    }
    ...
    def legalNeighbors: List[(Block, Move)] = {
      neighbors.filter{case (block: Block, move: Move) => block.isLegal}
    }
